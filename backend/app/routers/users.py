...
from app.schemas.schema import (
    ...
    UpdateBioRequest,
    UpdateHomeGymRequest,
    UpdateFavoritedRouteRequest,
)
...
@router.post("/login")
def login(
    form_data: OAuth2PasswordRequestForm = Depends(), db: DBSession = Depends(get_db)
):
    login_type = form_data.username.strip()
    user = (
        db.query(User)
        .filter(or_(User.email.ilike(login_type), User.username.ilike(login_type)))
        .first()
    )
...
def get_me_stats(
    ...
    total_sessions = (
        db.query(TrainingSession).filter(TrainingSession.user_id == user_id).count()
    )
    ...
    boulder_grades = [
        ...
        if a.route.grade.strip().upper().startswith("V")
    ]
    roped_grades = [
        a.route.grade
        for a in sent_attempts
        if not a.route.grade.strip().upper().startswith("V")
    ]
...
def update_favorited_route(
    body: UpdateFavoritedRouteRequest,
    db: DBSession = Depends(get_db),
    current_user=Depends(get_current_user),
):
    user = db.query(User).filter(User.id == current_user["user_id"]).first()
    ...
    if body.favorited_route_id is not None:
        route = (
            db.query(Route)
            .filter(Route.id == body.favorited_route_id, Route.user_id == user.id)
            .first()
        )
        if not route:
